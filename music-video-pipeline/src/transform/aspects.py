...
# --------------------------------------------------------------------------- #
# Framework integration: register the aspect transform as a groupoid citizen.
# --------------------------------------------------------------------------- #
from .core import REGISTRY, Recipe, RecipeStep, ScriptContext, Transformation  # noqa: E402


class RetargetAspect(Transformation):
    """Retarget a script to a different aspect ratio (16:9 <-> 9:16).

    Thin, journaled wrapper around :func:`transform_script`: the battle-tested
    scaling/fitting logic is unchanged; the framework records a journal so the
    operation is fully invertible (exact round-trip even through the lossy
    ``_ensure_text_fits`` step).
    """

    name = "retarget_aspect"
    description = "Retarget script between aspect ratios (16:9 <-> 9:16)"

    def run(self, ctx: ScriptContext, selector: dict, params: dict) -> None:
        target = params.get("target_aspect")
        if not target:
            raise ValueError("retarget_aspect requires 'target_aspect'")
        source = params.get("source_aspect")
        if not source and ctx.project_dir is not None:
            source = detect_aspect_ratio(ctx.project_dir)
        if not source:
            raise ValueError("retarget_aspect requires 'source_aspect' (or a project_dir to detect)")
        if source == target:
            raise ValueError(f"Source and target aspect ratios are the same: {target}")
        transformed = transform_script(ctx.script, source, target, project_dir=ctx.project_dir)
        ctx.script.clear()
        ctx.script.update(transformed)


REGISTRY.register(RetargetAspect())


def retarget_aspect_recipe(target_aspect: str, source_aspect: Optional[str] = None) -> Recipe:
    """Build a single-step recipe for aspect retargeting."""
    params: dict = {"target_aspect": target_aspect}
    if source_aspect:
        params["source_aspect"] = source_aspect
    return Recipe(
        name="retarget_aspect",
        steps=[RecipeStep(transform="retarget_aspect", params=params)],
    )
