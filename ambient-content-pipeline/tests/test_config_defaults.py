...
def test_auto_contrast_color_dark_bg() -> None:
    assert auto_contrast_color("#000000") == "#FFFFFF"
    assert auto_contrast_color("#333333") == "#FFFFFF"
    assert auto_contrast_color("#4A90E2") == "#FFFFFF"
    assert auto_contrast_color("#E67E22") == "#FFFFFF"
    assert auto_contrast_color("#8E44AD") == "#FFFFFF"
    assert auto_contrast_color("#C0392B") == "#FFFFFF"


def test_auto_contrast_color_light_bg() -> None:
    assert auto_contrast_color("#FFFFFF") == "#000000"
    assert auto_contrast_color("#FFFFCC") == "#000000"
    assert auto_contrast_color("#CCCCCC") == "#000000"
    assert auto_contrast_color("#FFB6C1") == "#000000"
    assert auto_contrast_color("#FFFACD") == "#000000"


def test_auto_contrast_color_invalid() -> None:
    assert auto_contrast_color("") == "#FFFFFF"
    assert auto_contrast_color("#ABC") == "#FFFFFF"
    assert auto_contrast_color("#12345") == "#FFFFFF"


def test_companion_color_returns_hex() -> None:
    result = companion_color("#E67E22")
    assert result.startswith("#")
    assert len(result) == 7


def test_companion_color_different_from_input() -> None:
    result = companion_color("#E67E22")
    assert result != "#E67E22"


def test_companion_color_invalid() -> None:
    assert companion_color("") == "#FFFFFF"
    assert companion_color("#ABC") == "#FFFFFF"


def test_ace_step_constants() -> None:
    assert isinstance(ACE_STEP_PYTHON, str)
    assert "ace_step_env" in ACE_STEP_PYTHON
    assert isinstance(ACE_STEP_SCRIPT, str)
    assert ACE_STEP_SCRIPT.endswith("run_ace_step_pipe.py")
    assert ACE_STEP_DEFAULT_STEPS == 20
    assert ACE_STEP_DEFAULT_GUIDANCE == 7.0
    assert ACE_STEP_MUSIC_VOLUME == 0.3
    assert ACE_STEP_TTS_DELAY == 0.5
    assert ACE_STEP_TAIL_MAX == 3.0
    assert ACE_STEP_TAIL_RATIO == 0.25
    assert ACE_STEP_TIMEOUT == 600
